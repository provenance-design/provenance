...
function RadialCanvas({ onOpenItem }) {
  ...
  const stateRef = useRef({
    ...
    activeTypes: null, cx: 0, cy: 0, radius: 0
  });
  ...
  useEffect(() => {
    ...
    S.cx = cx; S.cy = cy; S.radius = radius;
    ...
    // Group by discipline, sort by connection count within each group
    const discGroups = {};
    for (const n of S.nodes) {
      if (!discGroups[n.disc]) discGroups[n.disc] = [];
      discGroups[n.disc].push(n);
    }
    // Sort disciplines by size (largest first) for visual balance
    const discOrder = Object.keys(discGroups).sort((a, b) => discGroups[b].length - discGroups[a].length);
    // Within each discipline, sort by connection count descending
    for (const d of discOrder) {
      discGroups[d].sort((a, b) => b.nc - a.nc);
    }

    // Place nodes on circle perimeter, grouped by discipline
    const totalNodes = S.nodes.length;
    const gapAngle = 0.025; // gap between discipline segments
    const totalGap = gapAngle * discOrder.length;
    const availableAngle = Math.PI * 2 - totalGap;
    let currentAngle = -Math.PI / 2; // start at top

    const discSegments = []; // store segment info for labels
    for (const disc of discOrder) {
      const group = discGroups[disc];
      const segAngle = availableAngle * (group.length / totalNodes);
      const segStart = currentAngle;
      // Place most-connected nodes at centre of segment
      const mid = Math.floor(group.length / 2);
      const ordered = [];
      for (let i = 0; i < group.length; i++) {
        // Alternate placing from centre outward
        if (i % 2 === 0) ordered.push(group[mid + Math.floor(i / 2)]);
        else ordered.push(group[mid - Math.ceil(i / 2)]);
      }
      const validOrdered = ordered.filter(Boolean);
      for (let i = 0; i < validOrdered.length; i++) {
        const angle = segStart + (i + 0.5) * segAngle / validOrdered.length;
        validOrdered[i].x = cx + radius * Math.cos(angle);
        validOrdered[i].y_pos = cy + radius * Math.sin(angle);
        validOrdered[i].angle = angle;
      }
      const segMid = segStart + segAngle / 2;
      discSegments.push({ disc, midAngle: segMid, startAngle: segStart, endAngle: segStart + segAngle });
      currentAngle += segAngle + gapAngle;
    }

    // Store segments for label drawing
    S.discSegments = discSegments;
    ...
    function draw() {
      ...
      // Draw connections as quadratic Bezier curves through centre area
      for (const e of S.edges) {
        const sid = e.source.id, tid = e.target.id;
        ...
        if (!matchesFilter) {
          ctx.globalAlpha = 0.006; ctx.strokeStyle = '#333'; ctx.lineWidth = 0.2;
        } else if (activeId && !isActive) {
          ctx.globalAlpha = 0.015; ctx.strokeStyle = '#444'; ctx.lineWidth = 0.3;
        } else if (isActive && matchesFilter) {
          ctx.globalAlpha = 0.85;
          ctx.strokeStyle = '#E8E4DC';
          ctx.lineWidth = 1.8;
          // Glow effect
          ctx.shadowColor = '#E8E4DC';
          ctx.shadowBlur = 6;
        } else {
          // Ambient state — warm golden tones
          const ambientAlpha = typeFilter ? 0.18 : 0.09;
          ctx.globalAlpha = ambientAlpha;
          ctx.strokeStyle = TYPE_COLORS[e.type] || '#887766';
          ctx.lineWidth = typeFilter ? 0.6 : 0.35;
        }

        // Quadratic Bezier through a point offset from centre
        const sx = e.source.x, sy = e.source.y_pos;
        const tx = e.target.x, ty = e.target.y_pos;
        // Control point: pull toward centre with some offset for visual variety
        const midX = (sx + tx) / 2, midY = (sy + ty) / 2;
        const pull = 0.65; // how much to pull toward centre
      ...
      // Draw discipline segment labels at outer edge
      ctx.globalAlpha = 0.35;
      ctx.fillStyle = '#999';
      ctx.font = "9px 'DM Sans', -apple-system, sans-serif";
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      const labelR = radius + 28;
      for (const seg of S.discSegments) {
        const lx = cx + labelR * Math.cos(seg.midAngle);
        const ly = cy + labelR * Math.sin(seg.midAngle);
        ctx.save();
        ctx.translate(lx, ly);
        // Rotate label to follow the circle
        let rot = seg.midAngle;
        if (rot > Math.PI / 2 && rot < Math.PI * 1.5) rot += Math.PI; // flip for readability
      ...
      // Draw nodes
      for (const n of S.nodes) {
        const r = Math.sqrt(n.nc) * 2.2 + 2.5;
        ...
        if (filteredOut) {
          ...
          ctx.globalAlpha = dimmed ? 0.08 : isActive ? 1 : isNb ? 0.85 : typeFilter ? 0.65 : 0.55;
        }
        ctx.fillStyle = DISC_COLORS[n.disc] || '#999';

        // Subtle glow on nodes
        if (!filteredOut && !dimmed) {
          ctx.shadowColor = DISC_COLORS[n.disc] || '#999';
          ctx.shadowBlur = isActive ? 12 : isNb ? 6 : 2;
        }
        ctx.beginPath(); ctx.arc(n.x, n.y_pos, r, 0, Math.PI * 2); ctx.fill();
        ctx.shadowBlur = 0;

        if (isActive) {
          ctx.globalAlpha = 0.4; ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke();
          ctx.globalAlpha = 0.1; ctx.beginPath(); ctx.arc(n.x, n.y_pos, r + 8, 0, Math.PI * 2); ctx.fill();
        }

        // Labels
        if (!filteredOut && (isActive || isNb)) {
          ctx.globalAlpha = isActive ? 1 : 0.7;
          ctx.fillStyle = '#E8E4DC';
          ctx.font = (isActive ? '11px' : '9px') + " 'DM Sans', -apple-system, sans-serif";
          ctx.textAlign = 'center';
          const label = isActive ? `${n.t} (${n.nc})` : n.t.slice(0, 24);
          ctx.fillText(label, n.x, n.y_pos - r - 5);
        }
      }
    ...
    const onMove = (e) => {
      if (S.dragging) {
        ...
        let best = null, bestD = 20 / S.scale;
        for (const n of S.nodes) {
          const dist = Math.sqrt((n.x - mx) ** 2 + (n.y_pos - my) ** 2);
    ...
    const onResize = () => {
      ...
      const newCx = nW / 2, newCy = nH / 2;
      const newRadius = Math.min(nW, nH) * 0.38;
      const scaleRatio = newRadius / S.radius;
      for (const n of S.nodes) {
        n.x = newCx + (n.x - S.cx) * scaleRatio;
        n.y_pos = newCy + (n.y_pos - S.cy) * scaleRatio;
      }
      for (const seg of S.discSegments) {
        // Segment angles stay the same, labels recompute in draw
      }
      S.cx = newCx; S.cy = newCy; S.radius = newRadius;
      draw();
    };
          ...
          <div style={{ maxHeight: 200, overflow: 'auto' }}>
            {neighbors.slice(0, 18).map(n => (
              <div key={n.id} style={{ fontSize: '11px', color: '#AAA', padding: '2px 0', borderBottom: '1px solid #2A2E34', cursor: 'pointer' }}
                onClick={() => { S.selected = n; setPanelNode(n); if (drawRef.current) drawRef.current(); }}>
    ...
    return (
      <div style={{ width: '100vw', height: '100vh', overflow: 'hidden', background: '#1E2228' }}>
        ...
        <button onClick={() => { setView('featured'); setSelectedItem(null); }} style={{
          ...
          color: 'rgba(255,255,255,0.4)', cursor: 'pointer', backdropFilter: 'blur(12px)', borderRadius: 10,
        }}>← Back</button>
